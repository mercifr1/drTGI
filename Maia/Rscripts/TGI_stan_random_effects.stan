...
transformed parameters{
  ...
  for( j in 1:J){
   kg[j] = exp(log(tvkg)+eta_kg[j]*omega_kg);
   ks0[j] =exp(log(tvks0)+eta_ks0[j]*omega_ks0);
   gamma[j] =exp(log(tvgamma)+eta_gamma[j]*omega_gamma);
   alpha[j]= exp(log(tvalpha)+eta_alpha[j]*omega_alpha);
...
model {
  ...
  tvkg~lognormal(1,0.5);
  tvks0~lognormal(-0.36,1);
  tvgamma~lognormal(1,0.5);
  tvalpha~lognormal(1,0.5);
  tvbase~lognormal(55,5);
  ...
  for(j in 1:J){
    ...
    eta_kg ~ normal(0,5);
    eta_ks0 ~ normal(0,5);
    eta_gamma ~ normal(0,5);
    eta_alpha ~ normal(0,5);
    eta_base ~ normal(0,5);

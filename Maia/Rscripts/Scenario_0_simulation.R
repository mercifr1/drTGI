...
replicate_balance_data<-replicate(2,{
#dose-cohorts
...
# generate the number of individuals per cohor
...
final_balance_nadir<-final_balance_design_matrix %>%
  ...
  ungroup%>%filter(FLAG_DVMIN==0)
  as.data.frame()

  
  #'plot
p_balance<-ggplot(filter(final_balance_nadir,REP==1),aes(TIME,DV,group=ID,color=factor(DOSE)))+
    geom_line(size=1)+
    theme_bw()+
    ylab("SLD")
p_balance

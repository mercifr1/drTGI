...
$CAPTURE 
KS KS0 KG GAMA EPS(1) RESP_0 
'

#' Compile the model
#' ------------------------------------------------------
set.seed(123)
ODEmod2<-mcode("ODE2",ODEcod2)
...
indf1<-tibble(ID=1:nInd) %>% 
  split(.$ID) %>% 
  map_dfr(., ~funtime_ODE(1,1/10), .id="ID")
head(indf1,n=10)
...
flt_ODE<-indf1 %>%
  mutate(time=ty,T=ty, ID=as.numeric(as.character(ID)),
...
#' **1. For the flat dose**
set.seed(123)
snd.flt_ODE2<-mrgsim_d(ODEmod, as.data.frame(flt_ODE),carry.out="T") %>% 
  as.data.frame()
head(snd.flt_ODE2,n=20)

set.seed(123)
mrgsim_d(ODEmod, as.data.frame(flt_ODE), carry.out="T") %>%  plot(RESP~T)
...
#' 
set.seed(123)
snd.stepup0_ODE<-mrgsim_d(ODEmod, as.data.frame(stepup0_ODE), carry.out="T") %>% 
  as.data.frame()

head(snd.stepup0_ODE,n=10)

mrgsim_d(ODEmod, as.data.frame(stepup0_ODE), carry.out="T,KS,DOSE,RESP_0")%>%plot(RESP~T)


#' **3. For the 5  dose levels**
#' 
set.seed(123)
snd.stepup1_ODE<-mrgsim_d(ODEmod, as.data.frame(stepup1_ODE), carry.out="KS,DOSE,RESP_0,BASE") %>% 
  as.data.frame()

head(snd.stepup1_ODE,n=10)

mrgsim_d(ODEmod, as.data.frame(stepup1_ODE), carry.out="KS,DOSE,RESP_0,T")%>%plot(RESP~time)

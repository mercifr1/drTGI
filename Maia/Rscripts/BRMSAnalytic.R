source("C:/Users/muresai1/Desktop/drTGI/Maia/Rscripts/SimAnalytical.R")
...
#' Define the model structure and parameters < random effects>
analytic2 <- bf(RESP ~ exp(lBAS) *  exp( (exp(lKG)/100) * T - ( (exp(lKS0)/100) *log(DOSE)/ (exp(lGAMA)/100) ) *( 1 - exp((exp(lGAMA)/100)*T)))  ,
                nl = TRUE, 
                lBAS ~ 1+ (1|ID),
                lKG ~ 1 + (1 | ID),
                lKS0 ~ 1 + (1 | ID),

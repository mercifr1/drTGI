...
dat <- list(Nt = nrow(theoph.1),#total number of observations including time=0
            ts = theoph.1 %>% pull(Time) %>% `[`(-1),# times values for solve ODE without time 0
            y_init = c(theoph.1 %>% pull(DoseMass) %>% head(1), # the initial value for concentration and DoseMass
                       theoph.1 %>% pull(conc) %>% head(1)), # y_init[1]=DOSEMAss init and y_init[2]=c_init
            y = theoph.1 %>% pull(conc) %>% `[`(-1)) # response value : concentration without c_init
fit <- stan(file = "C:/Users/muresai1/Desktop/drTGI/Maia/Rscripts/theophylline_single_inidividual.stan",
            data = dat,
            iter = 1000, chains = 1)
...
dat <- list(N = theoph %>% pull(Subject) %>% unique %>% length,
            ...
            y_init = y_init,
            y = y)
fit <- stan(file = "C:/Users/muresai1/Desktop/drTGI/Maia/Rscripts/theophylline_hierarchical.stan",
            data = dat,
            iter = 1000, chains = 1)
